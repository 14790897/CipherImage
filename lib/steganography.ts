...
// 生成伪随机像素访问顺序
function getPixelOrder(width: number, height: number, key: string): [number, number][] {
...
// 将文本嵌入图片
export function embedTextInImage(imageData: ImageData, secretText: string, key: string): ImageData {
...
// 从图片中提取文本
export function extractTextFromImage(imageData: ImageData, key: string): string {
  const { width, height, data } = imageData
  const coords = getPixelOrder(width, height, key)
  let bits = ""
